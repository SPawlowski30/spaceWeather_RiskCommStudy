...
base_url = "https://www.ngdc.noaa.gov/stp/space-weather/swpc-products/weekly_reports/PRFs_of_SGD/"

# list of years 2020 - 2025
years = list(range(2020, 2026))  # 2024 to 2025
...
def get_text_files(url):
    response = requests.get(url)
    soup = BeautifulSoup(response.text, "html.parser")
    return [urllib.parse.urljoin(url, a["href"]) for a in soup.find_all("a", href=True) if a["href"].endswith(".pdf")]
